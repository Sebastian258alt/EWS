 ...
 *
 * ACTIVATION — tap the small 🐞 button (bottom-right corner) to open/close
 * the panel any time. It also auto-opens if either of these is true:
 *   - URL has ?debug=1   e.g. https://yoursite.com/?debug=1
 *   - localStorage.setItem('ews_debug', '1')  (persists across reloads —
 *     set automatically once you tap the 🐞 button, cleared on close)
 *
...
(function () {
  ...
  function buildPanel() {
    ...
    btnWrap.appendChild(makeBtn('✕', closePanel));
  ...
  function openPanel() {
    try { window.localStorage.setItem('ews_debug', '1'); } catch (e) {}
  ...
  // ---- Floating debug button (always present, opens the panel on tap) ----
  function buildButton() {
    var btn = document.createElement('button');
    btn.id = 'ews-debug-fab';
    btn.type = 'button';
    btn.title = 'Debug';
    btn.textContent = '🐞';
    btn.style.cssText = [
      'position:fixed', 'right:10px', 'bottom:10px', 'width:34px', 'height:34px',
      'border-radius:50%', 'border:1px solid #555', 'background:#222', 'color:#eee',
      'font-size:16px', 'line-height:1', 'z-index:2147483646', 'opacity:0.55',
  ...
  if (isEnabled()) openPanel();

  window.EWSDebug = { logs: logs, checkSW: checkSW, open: openPanel, close: closePanel, toggle: togglePanel };
})();
