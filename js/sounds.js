...
(function () {
  ...
  // ── Bootstrap ────────────────────────────────────────────
  // app.js declares `const SFX = {...}` at the top level of its own
  // <script>. That's a global LEXICAL binding shared across scripts —
  // it is NOT the same thing as `window.SFX`. Every call site in the
  // app (lesson.js, quiz.js, game.js, chat.js, ui-sounds.js, etc.)
  // calls the bare `SFX.click()` identifier, which resolves to that
  // lexical binding — so simply doing `window.SFX = <new object>`
  // here, as before, never actually reached any of those call sites;
  // they kept using app.js's basic beep object forever, silently.
  //
  // Fix: mutate the SAME object the bare `SFX` identifier already
  // points to, so every existing call site — guarded or not — starts
  // using this richer engine the instant this script runs. Also keep
  // window.SFX pointing at it, for the few call sites that do guard
  // on `window.SFX` first.
  if (typeof SFX !== 'undefined') {
    Object.assign(SFX, _richSFX);
    window.SFX = SFX;
  } else {
    // Defensive fallback — app.js didn't define SFX for some reason.
    window.SFX = _richSFX;
  }
