...
(function () {
  ...
  function createSettingsPanel() {
    if (document.getElementById('ews-audio-panel')) return;
    const p = getPrefs();
    const panel = document.createElement('div');
    panel.id = 'ews-audio-panel';
    panel.style.cssText = `
      ...
      font-family:inherit; color:var(--text,#fff); font-size:.9rem;
    `;
    panel.innerHTML = `
      <div style="font-weight:700;margin-bottom:12px;font-size:1rem">🔊 Audio Settings</div>
      <label style="display:flex;align-items:center;gap:10px;margin-bottom:10px;cursor:pointer">
        <input type="checkbox" id="aset-mute" ${p.muted?'checked':''}> Mute all
      </label>
      <label style="display:flex;align-items:center;gap:10px;margin-bottom:10px;cursor:pointer">
        <input type="checkbox" id="aset-vmute" ${p.voiceMuted?'checked':''}> Mute Flowy's voice
      </label>
      <label style="display:flex;align-items:center;gap:10px;margin-bottom:10px;cursor:pointer">
        <input type="checkbox" id="aset-ambient" ${p.ambientOff?'':'checked'}> Ambient music
      </label>
      <div style="margin-bottom:6px">🎵 Music volume</div>
      <input type="range" id="aset-ambvol" min="0" max="100" value="${Math.round((p.ambVol||0.18)*100)}" style="width:100%;accent-color:var(--blue,#3d7eff)">
      <div style="margin:10px 0 6px">🔊 SFX volume</div>
      <input type="range" id="aset-sfxvol" min="0" max="100" value="${Math.round((p.sfxVol||0.8)*100)}" style="width:100%;accent-color:var(--blue,#3d7eff)">
      <div style="margin:10px 0 6px">🐥 Flowy voice</div>
      <input type="range" id="aset-voicevol" min="0" max="100" value="${Math.round((p.voiceVol||0.85)*100)}" style="width:100%;accent-color:var(--teal,#1de9b6)">
    `;
    document.body.appendChild(panel);

    // Wire up controls
    panel.querySelector('#aset-mute').addEventListener('change', e => {
      const pr = getPrefs(); pr.muted = e.target.checked; savePrefs(pr); applyVolume();
    });
    panel.querySelector('#aset-vmute').addEventListener('change', e => {
      const pr = getPrefs(); pr.voiceMuted = e.target.checked; savePrefs(pr);
    });
    panel.querySelector('#aset-ambient').addEventListener('change', e => {
      const pr = getPrefs(); pr.ambientOff = !e.target.checked; savePrefs(pr);
      if (pr.ambientOff) stopAmbient(); else startAmbient();
    });
    panel.querySelector('#aset-ambvol').addEventListener('input', e => {
      const pr = getPrefs(); pr.ambVol = e.target.value / 100; savePrefs(pr);
      if (ambientGain) ambientGain.gain.setTargetAtTime(pr.ambVol, ctx.currentTime, 0.05);
    });
    panel.querySelector('#aset-sfxvol').addEventListener('input', e => {
      const pr = getPrefs(); pr.sfxVol = e.target.value / 100; savePrefs(pr);
      if (sfxGain) sfxGain.gain.setTargetAtTime(pr.sfxVol, ctx.currentTime, 0.05);
    });
    panel.querySelector('#aset-voicevol').addEventListener('input', e => {
      const pr = getPrefs(); pr.voiceVol = e.target.value / 100; savePrefs(pr);
    });
  }
  ...
  window.toggleAudioSettings = function() {
    initCtx();
    createSettingsPanel();
    const panel = document.getElementById('ews-audio-panel');
