...
function showPronWord() {
  ...
  // Clear previous feedback on word change
  const fb = document.getElementById('pronFb');
  if (fb) { fb.textContent = ''; fb.className = 'pron-fb'; }
}
...
function renderDifficult() {
  ...
  el.innerHTML = DIFFICULT_SOUNDS.map(s => `
    <div class="diff-card">
      ...
      <div class="diff-eg">${currentLang === 'pt' || currentLang === 'mz' ? 'Exemplos' : 'Examples'}: <strong>${s.eg}</strong></div>
