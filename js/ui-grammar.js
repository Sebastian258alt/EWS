...
function gramCard(g) {
  ...
  window._gramQuizRegistry = window._gramQuizRegistry || {};
  window._gramQuizRegistry[g.id] = { title: g.title, quiz: g.quiz };
  const isPt = currentLang === 'pt' || currentLang === 'mz' || currentLang === 'pt-pt';
  ...
  // Show bilingual sub if available
  const subText = g.subPt && isPt ? g.subPt : g.sub;
  const titleText = g.titlePt && isPt ? g.titlePt : g.title;
  return `<div class="gram-card" id="gc-${g.id}">
    <div class="gc-hdr" onclick="toggleGC('${g.id}')">
      <div class="gc-icon">${g.icon}</div>
      <div class="gc-meta">
        <div class="gc-title">${titleText}</div>
        <div class="gc-sub">
          ${subText}
    ...
    <div class="gc-body" id="gcb-${g.id}" style="display:none">
      <div class="gram-rtitle">📐 ${isPt ? 'Estrutura' : 'Structure'}</div>
      <div class="gram-formula">${g.formula}</div>
      <div class="gram-rtitle">✅ ${isPt ? 'Usos' : 'Uses'}</div>
      <div class="gram-uses">${usesHTML}</div>
      <div class="gram-rtitle">💬 ${isPt ? 'Exemplos (EN 🇬🇧 + PT 🇲🇿)' : 'Examples (EN 🇬🇧 + PT 🇲🇿)'}</div>
      <div class="gram-examples">${exHTML}</div>
      <div class="gram-rtitle">🏷️ ${isPt ? 'Palavras-Chave (Signal Words)' : 'Signal Words'}</div>
      <div class="gram-sigs">${sigHTML}</div>
      <div class="gram-tip">${g.tip}</div>
      <div class="gram-rtitle">⚠️ ${isPt ? 'Erros Comuns' : 'Common Mistakes'}</div>
      <div class="gram-mistakes">${mistHTML}</div>
      <button class="gram-quiz-btn" onclick="_launchGramQuiz('${g.id}')">
        🎯 ${isPt ? 'Praticar com Quiz' : 'Practice Quiz'} (+15 XP)
      </button>
    </div>
  </div>`;
}
