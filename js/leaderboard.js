...
const Leaderboard = (function () {
  ...
  async function fetchFromSupabase() {
    ...
    // Try user_profiles first (more common name)
    let data = null;
    let error = null;
    let usedTable = null;
    ...
    try {
      const result = await sb
        .from('user_profiles')
        .select('user_id, name, avatar, xp, updated_at')
        .order('xp', { ascending: false })
        .limit(200);
      if (!result.error && result.data && result.data.length > 0) {
        data = result.data;
        usedTable = 'user_profiles';
        log(`Using user_profiles table, ${data.length} records`);
      } else {
      ...
      try {
        ...
        if (!result2.error && result2.data) {
          ...
          usedTable = 'user_progress';
          log(`Using user_progress table, ${data.length} records`);
        } else {
          throw new Error('Both tables failed');
        }
      } catch (err2) {
        log(`Failed to fetch from both tables: ${err2.message}`);
        return [];
      }
    }
