...
const Leaderboard = (function () {
  ...
  async function fetchFromSupabase() {
    ...
    // FIXED: this used to try a table called `user_profiles` first and
    // only fall back to `user_progress` if that failed. `user_profiles`
    // is a stale, legacy table left over from an earlier version of the
    // app (it still exists in Supabase, so the query "succeeded" and
    // returned only 6 old rows) — it is NOT written to by storage.js
    // anymore. `user_progress` is the real, current table: it's what
    // storage.js syncs every user to, and what the admin panel's Users
    // tab reads from (37 rows). Reading from the wrong table caused the
    // leaderboard to only show 6 users. Read directly from
    // `user_progress` so it matches everywhere else in the app.
    let data = null;
    let usedTable = 'user_progress';
    ...
    try {
      ...
      log(`Using user_progress table, ${data.length} records`);
    } catch (err) {
      log(`Failed to fetch user_progress: ${err.message}`);
      return [];
    }
  ...
  function renderRows(rows) {
    ...
    // SECURITY: r.name and r.avatar come from Supabase `user_progress.payload`,
    // which is writable (subject to RLS) by any authenticated account — treat
