...
let _pmCustom   = false;
let _pmMethod   = null;
let _pmPrevTab  = null; // so Flowy goes back to normal tips after the panel closes
...
function openPM() {
  ...
  if (window.Flowy?.setTab) {
    _pmPrevTab = document.querySelector('.tab.active')?.id?.replace('tab-', '') || 'home';
    window.Flowy.setTab('support');
  }
}
...
function closePM() {
  ...
  if (window.Flowy?.setTab && _pmPrevTab) window.Flowy.setTab(_pmPrevTab);
}
    ...
    <div class="pm-methods">
      ${Object.values(PM_METHODS).map(m => `
        <button class="pm-method-card" onclick="_pmSelectMethod('${m.id}')"
          ...
          <img class="pm-method-logo-img" src="${m.logo}" alt="${m.name}" width="44" height="44">
...
function _stepInstructions() {
  ...
  return `
    ...
    <div class="pm-method-hero" style="background:${method.bg};border-color:${method.color}">
      <img src="${method.logo}" alt="${method.name}" width="48" height="48">
      <div>
