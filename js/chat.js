...
function appendMessageEl(msg) {
  ...
  div.innerHTML = `
    ...
    ${isUser ? `<div class="chat-av-user">${(window.escapeHtml || String)((getCU()?.name || 'U')[0].toUpperCase())}</div>` : ''}
  `;
  win.appendChild(div);
...
function formatChatContent(text) {
  // SECURITY: text is either the AI reply or the user's own raw typed
  // message. Escape FIRST, then apply the markdown-lite substitutions —
  // otherwise a typed "<img src=x onerror=...>" would be inserted as
  // live HTML via chatWin's innerHTML. See SECURITY.md.
