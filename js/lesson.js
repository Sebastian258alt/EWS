...
// Step 2.3C — shared UI-generation counter (also touched by quiz.js, nav.js).
// Bumped whenever a new navigation/screen intent should invalidate any
// still-pending EWSEnsureUnit()/EWSEnsureAllUnits() continuation from an
// earlier action. Does NOT cancel the underlying lazy-loader load (Step
// 2.3A's Promise cache is untouched) — it only decides whether a resolved
// load's UI callback is still worth applying.
window.__EWS_UI_GEN = window.__EWS_UI_GEN || 0;
...
function openLesson(unitId, lessonId) {
  // Step 2.3C: this call represents the current navigation intent — bump
  // the generation so any *other* still-pending lesson/quiz continuation
  // (from an earlier click) is invalidated. Safe for the normal recursive
  // re-entry below too: the mismatch check only fires if something ELSE
  // incremented the counter in between, not from this bump itself.
  ...
  if (unit && unit._lazy && window.EWSEnsureUnit) {
    ...
    window.EWSEnsureUnit(unitId).then(() => {
      // Stale: the user moved on (closed the screen, opened a different
      // lesson/quiz, or switched tabs) before this unit finished loading.
      // The unit itself stays loaded/cached — only this obsolete UI
      // callback is dropped.
      if (window.__EWS_UI_GEN !== _gen) return;
      openLesson(unitId, lessonId);
    });
    return;
  }
...
function closeLessonScreen() {
  // Step 2.3C: explicit exit — invalidate any still-pending lazy load from
  // this or another lesson so it can't silently reopen after the user left.
  window.__EWS_UI_GEN = (window.__EWS_UI_GEN || 0) + 1;
  SFX.click();
  LS = null;
  document.querySelectorAll('.tab').forEach(t => t.classList.remove('on'));
