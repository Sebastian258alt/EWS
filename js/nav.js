...
(function () {
  ...
  function openLB() {
    // Step 2.5: same __EWS_UI_GEN guard as lesson.js/quiz.js/nav.js's goTab()
    // (2.3C) and index.html's quiz/lesson shims (3.1B). This tap is the
    // current intent — bump+snapshot so a stale open→close (below) can drop
    // the deferred Leaderboard.open() instead of leaving Leaderboard's
    // internal _isOpen stuck true with the modal already closed, which was
    // silently no-op'ing the next real tap.
    window.__EWS_UI_GEN = (window.__EWS_UI_GEN || 0) + 1;
  ...
  function closeLB() {
    // Step 2.5: explicit exit — invalidate any still-pending leaderboard
    // load so it can't silently call Leaderboard.open() into a modal the
    // user already closed.
    window.__EWS_UI_GEN = (window.__EWS_UI_GEN || 0) + 1;
    const m = document.getElementById('lb-modal');
