// ═══════════════════════════════════════════════════════════
//  🐥 FLOWY.JS — Floating mascot assistant (BILINGUAL v5.1)
//  English With Sebastian
//  v5.1 changes from v5.0:
//    - Wake word recognition always uses en-US (more accurate)
//    - Fixed duplicate patterns in matchesWakeWord
//    - New tab tips: lesson / support / placement (EN + PT)
//    - Balanced PT content to match EN entry counts
//    - Lesson message category for in-lesson reactions
//    - Exponential-backoff reconnect for SpeechRecognition
//    - Better lesson-flow event hooks (data-step, .lesson-*)
//    - Streak milestone trigger added to public API
//    - iOS SpeechRecognition guard (unreliable on Safari)
//    - Bubble auto-flip if near edge of viewport
// ═══════════════════════════════════════════════════════════
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 62 62" width="62" height="62" aria-hidden="true">
  ...
  const MSGS = {
    en: {
      ...
      // NEW: lesson flow reactions
      lesson: [
        'Let\'s crush this lesson! 💪', 'Focus mode: ON 🎯',
        'You\'ve got this! One step at a time 📚',
        'Deep breath… here we go! 🌟', 'Learning in progress… 🧠✨',
        'This lesson will make you stronger! 💪',
        'I\'m rooting for you! 🐥', 'Stay sharp! 🎯',
      ],
      streak: [
        'STREAK ON FIRE!! 🔥🔥🔥 Keep going!!',
        'Day after day — you\'re unstoppable! 🏆',
        'STREAK LEGEND!! 🌟 Nothing can stop you!',
        'Consistency is your superpower! ⚡',
        'Your streak says: you don\'t give up! 🔥',
      ],
      easter_dance: [
    ...
    pt: {
      ...
      // NEW: lesson flow reactions
      lesson: [
        'Vamos arrasar esta lição! 💪', 'Modo foco: LIGADO 🎯',
        'Tu consegues! Um passo de cada vez 📚',
        'Respira fundo… vamos lá! 🌟', 'Aprendizagem em curso… 🧠✨',
        'Esta lição vai tornar-te mais forte! 💪',
        'Estou a torcer por ti! 🐥', 'Mantém o foco! 🎯',
      ],
      streak: [
        'SEQUÊNCIA EM CHAMAS!! 🔥🔥🔥 Continua assim!!',
        'Dia após dia — és imparável! 🏆',
        'LENDA DA SEQUÊNCIA!! 🌟 Nada te pára!',
        'A consistência é o teu superpoder! ⚡',
        'A tua sequência diz: não desistes! 🔥',
      ],
      easter_dance: [
  ...
  let reconnectDelay  = 1000;   // NEW: for exponential backoff
  ...
  // ── AI upgrade layer ───────────────────────────────────────
  // bubbleGen is bumped on every showBubble() call. An in-flight AI
  // response tags itself with the generation it belongs to; if the
  // bubble has moved on (new state, closed, whatever) by the time the
  // AI answers, the stale response is silently dropped instead of
  ...
  function smartBubble(key, ms, fallback, extra) {
    const staticText = (fallback !== undefined && fallback !== null) ? fallback : rand(msgs(key), key);
    showBubble(staticText);
    if (ms) scheduleHide(ms);

    if (!window.FlowyAI || !window.FlowyAI.isEnabled()) return;
    const gen = bubbleGen; // showBubble() above already incremented it
    const lang = isPT() ? 'pt' : 'en';

    window.FlowyAI.getLine(key, { lang: lang, fallback: staticText, extra: extra || {} })
      .then((aiLine) => {
        if (!aiLine || gen !== bubbleGen) return; // stale or no upgrade — keep static text
  ...
  function buildFlowy() {
    ...
    if (isIOS()) {
      ...
      micBtn.title = isPT() ? 'Voz não suportada no iOS Safari' : 'Voice not supported on iOS Safari';
    } else {
  ...
  // AI upgrade layer for the MODAL text (parallel to smartBubble/bubbleGen,
  // but targeting #flowy-modal-msg instead of the floating bubble).
  let modalGen = 0;
  function smartModalText(key, staticText, extra) {
    ...
    const gen  = modalGen;
    const lang = isPT() ? 'pt' : 'en';
    window.FlowyAI.getLine(key, { lang: lang, fallback: staticText, extra: extra || { tab: currentTab } })
      .then((aiLine) => {
        if (!aiLine || gen !== modalGen) return;
  ...
  function handleModalAction(e) {
    ...
    if (action === 'lesson') {
      ...
      const line = jArr[jokeIndex.formal % jArr.length];
      jokeIndex.formal++;
      smartModalText('joke_formal', line);
    } else if (action === 'joke-informal') {
      const jArr = msgs('jokes_informal');
      const line = jArr[jokeIndex.informal % jArr.length];
      jokeIndex.informal++;
      ...
      smartModalText('tab_tip', line, { tab: currentTab });
    } else if (action === 'back') {
      smartModalText('wakeWord', rand(msgs('wakeWord'), 'wakeWord'));
    }
  }
  ...
  function openModal(msg, key) {
    ...
    const staticText = msg || rand(msgs('wakeWord'), 'wakeWord');
  ...
  function setState(state) {
    ...
    if (state === 'thinking') {
      ...
      smartBubble('speaking', 3200);
    } else if (state === 'happy') {
      smartBubble('happy', 3000);
    } else if (state === 'sad') {
      smartBubble('sad', 3500);
    } else if (state === 'roaming') {
      smartBubble('roaming', 2000);
    } else if (state === 'sleeping') {
      smartBubble('sleeping', 0);
    } else if (state === 'celebrating') {
      smartBubble('celebrating', 3000);
      spawnConfetti(50);
    } else if (state === 'excited') {
      smartBubble('excited', 3000);
    } else if (state === 'love') {
      smartBubble('love', 3000);
    } else if (state === 'surprised') {
      smartBubble('surprised', 2500);
    } else if (state === 'confused') {
      smartBubble('confused', 3000);
    } else {
      hideBubble();
    }
  }
  ...
  function onFlowyClick() {
    ...
    if (clickCount === 50 && !dancing) { triggerDance(); return; }
    if (clickCount === 100) { triggerShades(); return; }

    rapidClickTimer = setTimeout(() => { rapidClicks = 0; }, 1500);
    if (rapidClicks >= 5) {
      rapidClicks = 0;
      setState('excited');
      smartBubble('tickle', 2500);
  ...
  function triggerStreak(days) {
    setState('celebrating');
    spawnConfetti(days >= 30 ? 80 : 50);
    const msg = days
      ? (isPT() ? `🔥 ${days} DIAS DE SEQUÊNCIA!! Incrível! 🏆` : `🔥 ${days}-DAY STREAK!! Incredible! 🏆`)
      : rand(msgs('streak'), 'streak');
    showBubble(msg);
    scheduleHide(5000);
    clearTimeout(happyTimer);
    happyTimer = setTimeout(() => setState('idle'), 5000);
  ...
  function triggerLessonStep() {
    if (currentState === 'idle' || currentState === 'roaming') {
      showBubble(rand(msgs('lesson'), 'lesson'));
      scheduleHide(3000);
    }
  }
      ...
      const obs = new MutationObserver((mutations) => {
        mutations.forEach(m => {
          ...
          if (m.target.id === 'toast') {
            if (text.includes('level') || text.includes('nível')) triggerLevelUp();
            else if (text.includes('streak') || text.includes('sequência')) triggerStreak(0);
    ...
    const lessonObs = new MutationObserver((mutations) => {
      mutations.forEach(m => {
        if (m.type === 'attributes') {
          ...
          if (step && step !== '0') triggerLessonStep();
        }
        if (m.type === 'childList') {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true, video: false })
      .then(stream => {
        ...
        volumeInterval = setInterval(() => {
          ...
          lastVolume = data.reduce((a, b) => a + b, 0) / data.length;
        }, 100);
      })
      .catch(() => {});
  }
  ...
  function detectLoudAudio() { return lastVolume > 60; }
    ...
    if (authScreen && appScreen) {
      const obs = new MutationObserver(() => {
        ...
        if (onAuth) {
          ...
          scheduleTipCycle();
          if (!isIOS() && supportsWakeWord() && !wakeListening) startWakeListening();
        }
      });
  ...
  window.Flowy = {
    ...
    streak:         triggerStreak,         // NEW
    lessonStep:     triggerLessonStep,     // NEW
