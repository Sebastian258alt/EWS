...
// SECURITY (P0):
//  • 'pw' was never in this list (good — passwords never left the device
//    this way), and now the field doesn't exist on the user object at all.
//  • 'email' has been REMOVED from this list. `user_progress` has a public
//    read policy ("public leaderboard read" — for select using (true)) so
//    the whole `payload` jsonb, including any field listed here, is
//    world-readable by anyone with the public anon key. Syncing email
//    into it was an undisclosed personal-data exposure — the leaderboard
//    only ever needed name/avatar/xp. If a future feature needs email
//    server-side, add a *separate* table without a public select policy.
