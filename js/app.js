...
function launchApp() {
  ...
  // Render curriculum immediately (visible on home tab)
  if (window.renderCurriculum) window.renderCurriculum();

  // Leaderboard data can be fetched eagerly (it's async, no DOM cost)
  if (window.renderLB) window.renderLB();

  // Chat init is lightweight
  if (window.initChat) window.initChat();

  // Refresh stats/UI for current user
  if (window.refreshUI) window.refreshUI();

  // Navigate to home tab
  goTab('home', document.getElementById('bn-home'));

  // Grammar & Sounds render lazily on first tab visit (see nav.js)
  // This saves ~120ms of synchronous DOM work on every login
}
