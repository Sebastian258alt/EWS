...
// ─── OVERLAYS ─────────────────────────────────────────────
// ── PREMIUM MODAL — Mozambican Payment Flow ───────────────
const PM_CONFIG = {
  whatsapp : '+258840000000',        // ← substitui pelo teu número WhatsApp
  mpesa    : { number: '+258 84 XXX XXXX', label: 'M-Pesa (Vodacom)', icon: '📱' },
  emola    : { number: '+258 86 XXX XXXX', label: 'E-Mola (Emtel)',   icon: '📲' },
  plans    : { mensal: { label: 'Mensal',     amount: '199 MZN', period: '/mês' },
...
function closePM() {
  ...
  setTimeout(() => showPmStep(1), 400); // reset after close animation
}
...
function copyPayNumber() {
  const cfg = PM_CONFIG[_pmMethod];
  if (!cfg) return;
  navigator.clipboard.writeText(cfg.number.replace(/\s/g, '')).then(() => {
    showToast('📋 Número copiado!');
  }).catch(() => {
    showToast('📋 ' + cfg.number);
  });
}
