...
// ═══════════════════════════════════════════════════════════
//  P0 SECURITY: doLogin/doReg no longer compare or store a plaintext
//  password. Real accounts authenticate through Supabase Auth
//  (js/auth.js: signInWithPassword/signUpWithPassword). Two narrow,
//  clearly-flagged fallbacks exist only so nothing breaks:
//   1. Legacy accounts created before this hardening pass still have a
//      plaintext `pw` locally; on their next successful login they are
//      transparently migrated to a real Supabase Auth account and `pw`
//      is deleted (see Auth.migrateLegacyLocalUser).
//   2. If Supabase is completely unreachable/misconfigured, registration
//      falls back to a locally salted+hashed password (PBKDF2 via
//      security-utils.js) instead of plaintext — clearly logged as a
//      degraded offline mode.
//  See SECURITY.md for the full picture and remaining risks.
// ═══════════════════════════════════════════════════════════

function _finishLogin(user) {
  localStorage.setItem('ews4_cur', user.un);
  if (user.lang) {
  ...
  return (typeof pullUserFromSupabase === 'function' ? pullUserFromSupabase(user.un) : Promise.resolve())
    .then(() => {
...
async function doLogin() {
  ...
  const localUser = getUsers().find(u => u.un === un);

  // Legacy plaintext account (pre-hardening): verify locally, then
  // migrate transparently to Supabase Auth.
  if (localUser?.pw !== undefined) {
    if (localUser.pw !== pw) { showErr(); return; }
    err.style.display = 'none';
    if (window.Auth && window.isSupabaseConfigured?.()) {
      await window.Auth.migrateLegacyLocalUser(localUser, pw).catch(() => false);
    }
    if (!localUser.sessionToken) {
      localUser.sessionToken = generateSessionToken();
      localUser.tokenExpiry = Date.now() + 7 * 24 * 60 * 60 * 1000;
    }
    saveCU(localUser);
    await _finishLogin(getCU() || localUser);
    return;
  }

  // Offline PBKDF2 fallback account (created while Supabase was down).
  if (localUser?.pwHash) {
    const ok = window.verifyPassword ? await window.verifyPassword(pw, localUser.pwSalt, localUser.pwHash) : false;
    if (!ok) { showErr(); return; }
    err.style.display = 'none';
    await _finishLogin(localUser);
    return;
  }

  // Normal path: real Supabase Auth account (new signups, or a legacy
  // account already migrated — locally or on another device).
  if (!window.Auth || !window.isSupabaseConfigured?.()) {
    showErr(t ? t('googleNotConfigured') : 'Sign-in is temporarily unavailable — please try again shortly.');
    return;
  }
  const result = await window.Auth.signInWithPassword({ username: un, password: pw });
  if (!result.ok || !result.user) { showErr(); return; }
  err.style.display = 'none';
...
async function doReg() {
  ...
  const showErr = (msg) => { if (msg) err.textContent = msg; err.style.display = 'block'; };

  if (!name || !un || pw.length < 8) { showErr(); return; }
  if (window.isReasonableUsername && !window.isReasonableUsername(un)) {
    showErr(currentLang === 'pt'
      ? 'Username inválido: usa só letras, números, "_", "-" ou "." (3–32 caracteres).'
      : 'Invalid username: letters, numbers, "_", "-" or "." only (3–32 chars).');
    return;
  }
  if (email && window.isReasonableEmail && !window.isReasonableEmail(email)) {
    showErr(currentLang === 'pt' ? 'Email inválido.' : 'Invalid email address.');
    return;
  }
  const pwStrong = /[A-Z]/.test(pw) && /[a-z]/.test(pw) && /[0-9]/.test(pw);
  if (!pwStrong) { showErr(t('weakPass')); return; }
  const arr = getUsers();
  if (arr.find(u => u.un === un)) { showErr(t('userTaken')); return; }
  err.style.display = 'none';

  const cleanName = window.clampDisplayName ? window.clampDisplayName(name) : name.slice(0, 32);

  if (window.Auth && window.isSupabaseConfigured?.()) {
    const result = await window.Auth.signUpWithPassword({ name: cleanName, username: un, email, password: pw });
    if (!result.ok) {
      showErr(result.error === 'user_already_exists' || /already/i.test(result.error || '')
        ? t('userTaken')
        : (currentLang === 'pt' ? 'Não foi possível criar a conta. Tenta novamente.' : "Couldn't create the account. Please try again."));
      return;
    }
    const newU = result.user;
    newU.lang = currentLang;
    saveCU(newU);
    if (result.needsEmailConfirmation) {
      showToast(currentLang === 'pt'
        ? '📧 Confirma o teu email para activares a conta.'
  ...
  // Degraded offline fallback — Supabase unreachable/misconfigured.
  console.warn('[EWS] Supabase not configured — registering with a locally salted+hashed password instead of plaintext. This account will need to be migrated once Supabase is available.');
  const newU = mkUser(cleanName, un);
  if (window.hashPassword) {
    const { saltHex, hashHex } = await window.hashPassword(pw);
    newU.pwSalt = saltHex;
    newU.pwHash = hashHex;
  }
  newU.authProvider = 'local-hashed';
  newU.lang = currentLang;
...
function launchApp() {
  ...
  _ric(() => {
    ...
    if (window.EWSLoadFeature) {
      ...
      // flowy-ai.js (~2KB) — the AI upgrade layer flowy.js's smartBubble()
      // calls into. Loaded as its own tiny feature just before flowy.js so
      // window.FlowyAI exists once the mascot needs it. flowy.js already
      // guards every call with `window.FlowyAI &&`, so if this ever fails
      // to load, flowy.js simply keeps using its static message banks —
      // no functional gap, no crash, same idle-load-doesn't-block reasoning
      // as everything else in this block.
      window.EWSLoadFeature('flowy-ai', 'js/flowy-ai.js').catch(err => {
        console.error('[EnglishFlow] Failed to load flowy-ai.js:', err);
      });
