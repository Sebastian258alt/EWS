...
function refreshUI() {
  ...
  if (pXPNxt) pXPNxt.textContent = (lv.next - u.xp) + (window.currentLang && window.currentLang.startsWith('pt') ? ' XP para o próximo nível' : ' XP to next level');
  ...
  if (psQuiz) psQuiz.textContent = u.quizzesTaken || 0;
  const psPron = document.getElementById('ps-pron');
  if (psPron) psPron.textContent = u.pronP || 0;
  // Language badge
  const langBadge = document.getElementById('p-lang-badge');
  if (langBadge) {
    const langMap = {en:'🇺🇸 EN', 'en-gb':'🇬🇧 EN', mz:'🇲🇿 MZ', pt:'🇧🇷 PT', 'pt-pt':'🇵🇹 PT'};
    langBadge.textContent = langMap[window.currentLang] || '🌐 EN';
  }
