...
(function () {
  ...
  // ── About page ───────────────────────────────────────────
  // targetId: optional - renders into that element instead of tab-about
  // Used by About Panel (openAboutPanel calls renderAbout('aboutPanelContent'))
  window.renderAbout = function (targetId) {
    const el = document.getElementById(targetId || 'aboutPanelContent');
    if (!el) return;
    const d    = ABOUT_DATA;
