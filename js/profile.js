...
(function () {
  ...
  window.renderAbout = function () {
    ...
    const certHTML = (d.certifications || []).map(c =>
      `<div class="cert-card">
        ...
        <div class="cert-body">
          <div class="cert-title">${lang === 'pt' ? c.pt : c.en}</div>
    ...
    <div class="about-wrap">
      <div class="about-hero-card">
        ...
        <div class="about-avatar-ring">
          <div class="about-avatar"><img src="icons/creator-photo.jpg" alt="${d.name}" loading="lazy"></div>
        ...
        <div class="about-bio">${lang === 'pt' ? d.summary.pt : d.summary.en}</div>
      ...
      ${certHTML ? `<div class="about-section">
        <div class="about-section-title">${window.t ? t('certifications') : '🏅 Certifications'}</div>
      ...
      <div class="about-section">
        ...
        <div class="contact-cards">
          <div class="contact-card" onclick="navigator.clipboard.writeText('${d.email}').then(()=>showToast(lang==='pt'?'📋 Email copiado!':'📋 Email copied!'))">
            <span>✉️</span>
            <div><div class="contact-label">Email</div><div class="contact-val">${d.email}</div></div>
            <span class="contact-copy">📋</span>
          </div>
          ${d.email2 ? `<div class="contact-card" onclick="navigator.clipboard.writeText('${d.email2}').then(()=>showToast(lang==='pt'?'📋 Email copiado!':'📋 Email copied!'))">
            <span>✉️</span>
            <div><div class="contact-label">${lang === 'pt' ? 'Email Alternativo' : 'Alternative Email'}</div><div class="contact-val">${d.email2}</div></div>
            <span class="contact-copy">📋</span>
          </div>` : ''}
          <div class="contact-card" onclick="navigator.clipboard.writeText('${d.phone}').then(()=>showToast(lang==='pt'?'📋 Número copiado!':'📋 Number copied!'))">
            <span>📞</span>
            <div><div class="contact-label">Phone / WhatsApp</div><div class="contact-val">${d.phone}</div></div>
            <span class="contact-copy">📋</span>
          </div>
          ${d.phone2 ? `<div class="contact-card" onclick="navigator.clipboard.writeText('${d.phone2}').then(()=>showToast(lang==='pt'?'📋 Número copiado!':'📋 Number copied!'))">
            <span>📞</span>
            <div><div class="contact-label">${lang === 'pt' ? 'Telefone Alternativo' : 'Alternative Phone'}</div><div class="contact-val">${d.phone2}</div></div>
            <span class="contact-copy">📋</span>
          </div>` : ''}
          ${d.website ? `<div class="contact-card" onclick="window.open('${d.website}','_blank')">
            <span>🌐</span>
            <div><div class="contact-label">${lang === 'pt' ? 'Website' : 'Website'}</div><div class="contact-val">${d.website.replace('https://','')}</div></div>
            <span class="contact-copy">↗️</span>
          </div>` : ''}
        </div>
      </div>
    </div>`;
  ...
  function downloadCV() {
    ...
    const isPt = window.currentLang && window.currentLang.startsWith('pt') || window.currentLang === 'mz';
    try {
      const a = document.createElement('a');
      a.href = 'cv.pdf';
      a.download = 'Sebastiao_Nhamuneque_CV.pdf';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      if (window.showToast) showToast(isPt ? '📄 A descarregar o CV…' : '📄 Downloading CV…');
    } catch (e) {
      if (window.showToast) showToast(isPt ? '⚠️ Não foi possível abrir o CV.' : '⚠️ Could not open the CV.');
    }
  }
