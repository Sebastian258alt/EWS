...
let Q = {
  ...
  typesUsed:new Set(), // question types answered correctly, for missions
};
...
function startLesson(unitId, lessonId){
  // Step 2.3C: see js/lesson.js's openLesson for the full rationale — this
  // call is the current intent, so bump the shared generation to invalidate
  // any other pending lesson/quiz continuation, then guard our own deferred
  // continuation (if this unit needs a lazy load) against going stale too.
  window.__EWS_UI_GEN = (window.__EWS_UI_GEN || 0) + 1;
  const unit=UNITS.find(u=>u.id===unitId);
  if(unit && unit._lazy && window.EWSEnsureUnit){
    const _gen = window.__EWS_UI_GEN;
    window.EWSEnsureUnit(unitId).then(()=>{
      if (window.__EWS_UI_GEN !== _gen) return;
  ...
  Q={ qs:lesson.qs.map(_asMC), idx:0, score:0, combo:0, maxC:0, lessonId, unitId,
      ...
      shieldActive:false, speedBonus:0, versusMode:false, wrongStreak:{}, liveXP:0, typesUsed:new Set() };
  showQuiz();
}
window.startLesson=startLesson;

function startDailyChallenge(){
  // Step 2.3C: same token pattern as startLesson/openLesson.
  window.__EWS_UI_GEN = (window.__EWS_UI_GEN || 0) + 1;
  SFX.click(); let u=getCU(); if(!u) return;
  const today=new Date().toDateString();
  if(u.dailyChallengeDone===today){ showToast(t('alreadyDone')); return; }
  // Daily challenge mixes questions from every unit, so — unlike opening a
  // single lesson — it needs ALL units' real content loaded first.
  if(UNITS.some(un=>un._lazy) && window.EWSEnsureAllUnits){
    const _gen = window.__EWS_UI_GEN;
    showToast(t('loading')||'Loading…');
    window.EWSEnsureAllUnits().then(()=>{
      if (window.__EWS_UI_GEN !== _gen) return;
      startDailyChallenge();
    });
    return;
  ...
  Q={ qs:mixed, idx:0, score:0, combo:0, maxC:0, lessonId:null, unitId:null,
      ...
      versusGhost:ghost, versusMode:!!ghost, wrongStreak:{}, liveXP:0, typesUsed:new Set() };
  showQuiz();
}

function startQuizFromData(title, qs, xp, gramMode=false){
  // Step 2.3C: no lazy load happens here, but this is still a screen-owning
  // entry point — bump so any earlier still-pending lesson/quiz continuation
  // (e.g. a lesson unit that was mid-load) is invalidated instead of
  // clobbering this quiz once it resolves.
  window.__EWS_UI_GEN = (window.__EWS_UI_GEN || 0) + 1;
...
function closeQuiz(){
  // Step 2.3C: explicit exit — invalidate any still-pending lazy load
  // (this quiz's own, or any other lesson/quiz) so it can't silently pop
  // back up after the user has already left.
  window.__EWS_UI_GEN = (window.__EWS_UI_GEN || 0) + 1;
  clearInterval(_speedTimer);
...
function resumeLesson(){
  ...
  for(const unit of UNITS){ for(const lesson of unit.lessons){ if(!u.done.includes(lesson.id)){ if(typeof openLesson==='function') openLesson(unit.id,lesson.id); else startLesson(unit.id,lesson.id); return; } } }
