...
const Auth = (function () {
  ...
  // Whether an account's email is the app-invented placeholder rather
  // than something the person actually typed in.
  function hasSyntheticEmail(user) {
    if (!user?.email) return true;
    return user.email === syntheticEmail(user.un);
  }

  // Lets someone add/replace a real email from Settings, so Supabase's
  // "forgot password" flow can actually reach them later. Only works for
  // accounts that already have a live Supabase session (authProvider
  // 'supabase' or 'google') — local/legacy/offline-hashed accounts need
  // to log in normally first, which is what triggers their migration.
  async function updateAccountEmail(newEmail) {
    const sb = getClient();
    if (!sb) return { ok: false, error: 'not_configured' };
    if (!window.isReasonableEmail?.(newEmail)) return { ok: false, error: 'invalid_email' };
    ...
    // Supabase may require confirming the new address before it takes
    // effect; either way it's safe to remember locally what the person
    // asked for. It stays local-only (see storage.js SYNC_ALLOW).
    const cu = getCU();
    if (cu) { cu.email = newEmail; saveCU(cu); }
