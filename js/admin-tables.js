...
const ADMIN_TABLES = {
  ...
  user_progress: {
    // FIXED: this table is called `user_progress` in Supabase (see
    // supabase-migration/01_schema.sql and js/storage.js, which upserts
    // to .from('user_progress')). The key here is used verbatim as the
    // table name by AdminAPI (sb.from(tableName)), so a key of `users`
    // was querying a table that doesn't exist ("public.users" 404 /
    // PGRST205), when the real table is "public.user_progress".
    label: 'Users',
    group: 'Users & progress',
    pk: 'un',
    pkEditable: false, // ASSUMPTION: `un` (username) is created by the app at signup, not hand-picked here
    orderBy: 'updated_at',
    ascending: false, // most recently active users first
    fields: [
      { name: 'un', label: 'Username', type: 'text', required: true },
      { name: 'supabase_id', label: 'Supabase auth ID', type: 'text' },
      { name: 'payload', label: 'Progress data (JSON: xp, streak, hearts, gems, done, achs…)', type: 'json' },
      { name: 'updated_at', label: 'Last updated', type: 'text' },
      { name: 'online_at', label: 'Last sync ping', type: 'text' },
    ],
  },
