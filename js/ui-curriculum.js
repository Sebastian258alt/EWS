  ...
  UNITS.forEach((unit, ui) => {
    ...
    wrap.innerHTML = `
      <div class="unit-hdr" style="background:linear-gradient(135deg,${unit.color}18,${unit.color}08);border-color:${unit.color}33">
        ...
        <span class="u-badge ub-free">${badge}</span>
      </div>
      <div class="unit-prog">
...
function lnode(l, u, unit) {
  ...
  const isLocked   = !done && !allBefore;
  const cls        = done ? 'done' : isCurr ? 'curr' : isLocked ? 'lock' : 'curr';
  const action     = isLocked ? '' : `openLesson('${unit.id}','${l.id}')`;
...
// Exposed on window so ui-grammar.js (loaded in parallel) can write to it.
window._gramQuizRegistry = window._gramQuizRegistry || {};

function _launchGramQuiz(gramId) {
  const entry = window._gramQuizRegistry[gramId];
  if (!entry) return;
